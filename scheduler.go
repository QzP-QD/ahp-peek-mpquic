...
func (sch *scheduler) setup() {
	...
	sch.peekmemories = make(map[protocol.PathID]*PeekMemory)
	...
	for _, frame := range packet.frames {
		...
		case *wire.StreamFrame:
			if frame.FinBit {
				...
				os.Remove("/home/mininet/output/lin")
				os.Create("/home/mininet/output/lin")
...
func (sch *scheduler) sendPacket(s *session) error {
	...
	for {
		...
		if sch.SchedulerName == "rl" || sch.SchedulerName == "peek" {
		...
		if original_pth != nil {
			...
			if sch.SchedulerName == "peek" && (flag == tx || flag == normal) {
				sch.storePeekAction(s, original_pth.pathID, pkt, flag)
			}
		}
